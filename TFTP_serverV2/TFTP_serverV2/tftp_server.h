...
public:
	...
	struct ServerClient {
		...
		const char* ip;							//- ip adresas, pranesimu apie klienta rodymui
...
protected:
	...
	bool sendError(ServerClient* client, int error_no, const char* custom_message = "");
	bool disconnectClient(ServerClient* client);

	void clientStatus(ServerClient* client, const char* message);
